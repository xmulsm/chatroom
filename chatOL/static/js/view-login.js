...
var login = (function () {
    ...
    login = function () {
        ...
        var pwd_md5=hex_md5(pwd);    //密码用MD5加密传输
        if (!email || !pwd) {
        ...
        console.log(datas)
        $.ajax({
            url: '/accounts/login/' + stateMap.$rememberMe,
            type: 'post',
            data: datas,
            dataType: "html",
            success: function (result) {
                console.log(result);
                if (result== 0) {
                    showAlert(domMap.$alert, '登录成功！');
                    location.replace(root + "/chatroom/");
...
//                    }
                    ...
                    showAlert(domMap.$alert, '密码错误,请检查后重新登录');
                } else if (result == 11) {
                    $('.re-regist').removeClass('none')
                }else if(result==1){
                    showAlert(domMap.$alert, '用户不存在,请检查后重新登录');
                }else if(result==2){
                   showAlert(domMap.$alert, '用户已被锁定,请联系管理员');
                }
            }
        });
    };
...
//        console.log(password.split(""));
        ...
        // 新增校验规则
        var pwd_md5=hex_md5(password);    //密码用MD5加密传输
        var datas = regCheck(email, code, name, pwd_md5)
        if (!datas) {
            return false
        }
        console.log(datas)
...
//                        location.replace(root + '/paper');
//                    }
                    showAlert(domMap.$alert, '注册成功，请登录！');
                     document.title = "登录";
                    $('#login_box').show();
                    $('#reg_box').hide();
...
//                    stateMap.refuse = true
                    ...
                    showAlert(domMap.$alert, '用户名已存在，请直接登录，或者检查后重新注册！');
                }
            }
        });
    };
